      ...
      >
        <Skeleton borderRadius="2xl" boxSize={titleCard ? `150px` : `110px`} isLoaded={imageLoad}>
          <Image
            ...
            src={song.albumImageUrl || song.album.images.filter((image: any) => image.height > 109).slice(-1)[0].url}
          />
        </Skeleton>

        <Flex direction="column" ml={5} maxWidth="full" isTruncated>
          <Text isTruncated maxWidth="full" fontSize={titleCard ? { base: `2xl`, md: `3xl` } : { base: `xl`, md: `2xl` }} fontWeight="semibold">
            {`${song.name}${titleCard && !isPlaying ? ` - Paused` : ``}`}
          </Text>
          <Flex direction="column" color={useColorModeValue(`gray.600`, `gray.400`)} mt={2} isTruncated width="full" fontSize={{ base: `sm`, sm: `md` }}>
...
export const ArtistCard = ({ artist }: { artist: any }) => {
  ...
  return (
    ...
    >
      <Skeleton overflow="visible" maxWidth="150px" borderRadius="2xl" maxHeight="150px" boxSize="full" mx="auto" isLoaded={imageLoad}>
        <Image
