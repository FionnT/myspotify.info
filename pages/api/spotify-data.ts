...
async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  if (req.method === `GET`) {
    try {
      const { responseTracks, responseArtists, responseRecently, responseProfile } = await getSpotifyData(req.query.period, req.query.token)
      ...
      return res.status(200).json({ artists, songs, recentlyPlayed, profile })
    } catch (e:any) {
